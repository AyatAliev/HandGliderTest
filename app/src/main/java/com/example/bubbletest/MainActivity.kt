...
fun BubbleMapView(sdkContext: Context, navigationManager: NavigationManager) {
    var styleCloseable: AutoCloseable
        ...
        factory = { context ->
            MapView(context).apply {
                this.getMapAsync { map ->
                    ...
                    styleCloseable = builder.loadStyle(
                        File.fromAsset(sdkContext, "styles.2gis")
                    ).apply {
                        onResult {
                            map.style = it
                        }
        ...
        Row {
            Button(onClick = {
                ...
                val finishSearchPoint = RouteSearchPoint(
                    coordinates = GeoPoint(latitude = 55.3660267, longitude = 37.6671895)
                )
                ...
                routesFuture.onResult {
                    ...
                    navigationManager.simulationSettings.speedMode = SimulationSpeedMode(
                        speed = SimulationConstantSpeed(0.0)
                    )
